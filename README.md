# HelloID-Conn-SA-Full-AD-ReportAccountsPasswordExpireIn14Days
...
## Description
_HelloID-Conn-SA-Full-AD-ReportAccountsPasswordExpireIn14Days_ is a template designed for use with HelloID Service Automation (SA) Delegated Forms. It can be imported into HelloID and customized according to your requirements. 

This HelloID Service Automation Delegated Form provides an Active Directory report containing the user accounts with passwords that will expire in 14 days. The following options are available:
 1. Overview of AD user accounts with passwords expiring in 14 days
...
- **Password Expiration Window**: The report identifies accounts where passwords will expire within 14 days. It filters for enabled accounts that do not have "Password Never Expires" set.
- **Computed Expiry Date**: The report uses the `msDS-UserPasswordExpiryTimeComputed` AD attribute to determine the exact password expiration date for each account.
- **Date Formatting**: Expiry dates are formatted as dd-MM-yyyy for easy readability.

### Performance Considerations
- **Enabled Accounts Only**: The data source only queries enabled accounts with password expiration enabled to optimize performance and relevance.
- **OU Scoping**: The report searches through the organizational units (OUs) specified in the `AdUsersReportOu` variable. Limit the scope to relevant OUs to improve performance in large Active Directory environments.
